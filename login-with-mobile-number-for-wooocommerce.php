...
function sunarcwooc_phone_register_field() {?>
       <p class="form-row form-row-wide">
       <label for="reg_billing_phone"><?php _e( 'Phone', 'woocommerce' ); ?><span class="required">*</span></label>
       <input type="text" class="input-text" name="billing_phone" id="reg_billing_phone" onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))" maxlength="10" required="" value="<?php esc_attr_e( $_POST['billing_phone'] ); ?>" />
       </p>
       <div class="clear"></div>
       <?php
 }
 add_action( 'woocommerce_register_form_start', 'sunarcwooc_phone_register_field' );
...
* Phone Validating.
*/
function sunarcwooc_validate_phone_field( $username, $email, $validation_errors ) {
...
add_action( 'woocommerce_register_post', 'sunarcwooc_validate_phone_field', 10, 3 );
...
* Save Phone field.
*/
function sunarcwooc_save_phone_field( $customer_id ) {
    if ( isset( $_POST['billing_phone'] ) ) {
            // Phone input filed which is used in WooCommerce
            update_user_meta( $customer_id, 'billing_phone', sanitize_text_field( $_POST['billing_phone'] ) );
    }
}
add_action( 'woocommerce_created_customer', 'sunarcwooc_save_phone_field' );
...
* Change label.
*/
add_filter( 'gettext', 'sunarcwooc_change_registration_usename_label', 10, 3 );
function sunarcwooc_change_registration_usename_label( $translated, $text, $domain ) {
...
if (isset($_REQUEST['username']) && isset($_REQUEST['password'])) {
	...
	add_filter('authenticate', function($user, $email, $password){
		...
		if(preg_match("/^[0-9]{10}+$/", $_REQUEST['username'])) {
		  $type = 'phone';
		}
		elseif(filter_var($_REQUEST['username'], FILTER_VALIDATE_EMAIL)){
			$type = 'email';
		}
		else{
			$type = 'username';
...
*/
add_action( 'woocommerce_after_save_address_validation','sunarccustom_validation_phone',1,2 );
function sunarccustom_validation_phone( $user_id, $load_address ) {
